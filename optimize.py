...
def run(model, param_grid, X_train, y_train, X_test, y_test, scoring = 'f1', verbose = 0):
  ...
  grid_search = GridSearchCV(estimator=model, param_grid=param_grid, cv=5, scoring = scoring, verbose=verbose, n_jobs=-1)
  ...
  fn = conf_matrix[1][0]
  fn_rate = fn / (fn + conf_matrix[1][1] + conf_matrix[0][1] + conf_matrix[0][0]) * 100
  print("FN Rate   :", int(fn_rate), "%")
...
def print_statistics(df_metrics):
  ...
  # compute average fn rate
  fn = df_metrics['FN'].sum()
  tp = df_metrics['TP'].sum()
  tn = df_metrics['TN'].sum()
  fp = df_metrics['FP'].sum()
  fn_rate = fn / (fn + tp + tn + fp) * 100
  
  print(f"F1 Score  : {f1}")
  print(f"AUC Score : {roc_auc}")
  print(f"Accuracy  : {accuracy}")
  print(f"Precision : {precision}")
  print(f"FN Rate   : {int(fn_rate)} %")
